...
export const options = {
  scenarios: {
    ...
    detail: {
      executor: 'constant-arrival-rate', duration: '90s', rate: 50, timeUnit: '1s',  // TODO: use 200 req/s?
...
export function overview() {
  ...
  if (resp.status !== 200) {
    // TODO exec.test.abort(overviewUrl + ' for user ' + userIndex + ' with status ' + resp.status);
  }
  check(resp, {
...
export function detail() {
  ...
  if (resp.status !== 200) {
    // TODO exec.test.abort(detailUrl + ' for user ' + userIndex + ' with status ' + resp.status);
  }
  check(resp, {
    'is status 200': (r) => r.status === 200,
  });
